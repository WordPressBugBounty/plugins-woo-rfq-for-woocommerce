...
<form  style="display: block" class="woo_rfq_after_shop_loop_button"
      ...
      action='<?php echo wp_kses_post($product->add_to_cart_url()) ?>' method='post'>
                        <?php $nonce = wp_create_nonce('rfq_id_nonce');
                        wp_kses_post(wp_nonce_field('rfq_id_nonce')); ?>
                      ...
                      <input type='hidden' value='<?php echo wp_kses_post($product_type); ?>' name='product_type' id='product_type'/>
                        <input type='hidden' value='<?php echo wp_kses_post($product->get_id()); ?>' name='rfq_product_id'
                               id='rfq_product_id'/>
                       <?php if(!class_exists('GPLS_WOO_RFQ_PLUS')
                           || get_option("gpls_rfq_show_add_to_quote_qty","no")!="yes"): ?>
